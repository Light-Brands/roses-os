...
function translateHtml(html: string, t: Translation, locale: string): string {
  ...
  result = result.replace(
    '<title>International Aura and Dream School — Teacher Visual Aid Manual</title>',
    `<title>International Aura and Dream School — ${t.ui.teacherVisualAidManual}</title>`
  );

  // 3. Cover page
  result = result.replace(
    'International Aura and Dream School &middot; Teacher Visual Aid Manual',
    `International Aura and Dream School &middot; ${escapeHtml(t.ui.teacherVisualAidManual)}`
  );
  result = result.replace(
  ...
  result = result.replace(
    'Lineage: Berkeley Psychic Institute &rarr; Anastasia Plunk &rarr; Angelina Ataide &rarr; Escola da Aura &rarr; International Aura and Dream School',
...
const EN_SLIDES: Record<string, { concept: string; teachingText: string }> = {
  ...
  'l1-four-roses': { concept: 'Four Roses of Protection, Separation and Observation', teachingText: 'Roses are placed at the edges of the aura to serve as energetic sentinels.' },
